...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO MEMBER VALUES('MEM02','QWER1234','김영희','980909-234567',SYSDATE);
COMMIT;

UPDATE MEMBER SET MEMBER_SSN ='980808-2345678'
WHERE MEMBER_ID = 'MEM02';

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
--  데이터  삽입 확인
SELECT * FROM MEMBER;
INSERT INTO MEMBER VALUES('MEM03','ASDF1234','박철수','970606-1234567',DEFAULT);
COMMIT;

INSERT INTO MEMBER(MEMBER_ID,MEMBER_PWD,MEMBER_NAME,MEMBER_SSN)
VALUES('MEM04','12341234','이지연','000212-4568901');
COMMIT;


--주민 NUMBER x 앞이 0면 0이 사라짐
ROLLBACK;
...
-- 2. UNIQUE 제약조건 
-- 컬럼에 입력 값에 대해서 중복을 제한하는 제약조건 - JAVA.SET
...
--    USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE, --제약조건명     
    USER_ID VARCHAR2(20),
    USER_PWD VARCHAR2(30) ,
    USER_NAME VARCHAR2(30),
    GENDER VARCHAR2(10),
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT GRADE_LEVEL REFERENCES USER_GRADE
--  ,CONSTRAINT GRADE_LEVEL FOREINGN KEY(GRADE_CODE) REFERENCES USER_GRADE
);
...
CREATE TABLE USER_USED_FK2(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_KF2 REFERENCES USER_GRADE2 ON DELETE SET NULL
);
...
CREATE TABLE USER_USED_FK3(
  ...
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER
  ,CONSTRAINT GRADE_CODE_FK3 FOREIGN KEY (GRADE_CODE) REFERENCES USER_GRADE3
  ON DELETE CASCADE
);
...
ALTER TABLE EMPLOYEE ADD
CONSTRAINT FK_SAL_GRADE FOREIGN KEY(SAL_LEVEL) REFERENCES SAL_GRADE;
...
CONSTRAINT FK_NATIONAL FOREIGN KEY(NATIONAL_CODE) REFERENCES NATIONAL;
