...
<body>
    ...
    <pre>
    	...
    	쉽고, 표기법을 간단히 할 수 있도록 태그화(Tag Library)하여 표준으로 제공함.
    
    	(if,for,scope변수 선언, 데이터 파싱)
    </pre>
    ...
    <h3>1. 변수 선언( c:set )</h3>
    <pre>
    	- 변수를 선언하고 값을 초기화하는 태그(초기화 무조건 수행)
...
\
	...
	<pre>
		...
		- if문만 가능하고 else는 불가능(c:else가 존재하지 않음)
	...
	<h3>4. 조건물 -if ~ else if ~ else (c:choose, c:when, c:otherwise)</h3>
