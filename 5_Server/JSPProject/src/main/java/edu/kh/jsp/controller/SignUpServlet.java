...
//@WebServlet("요청주소") : Servlet클래스 등록 + 요청 주소 매핑

//http://localhost:10005/JSPProject/signUp?memberId=&memberPw=&memberName=&intro=
@WebServlet("/signUp")
...
//				이 떄, 문자 인코딩은 제출된 HTML파일의 문자 인코딩(charset)을 따름
...
//		Servlet으로 응답화면 만들기 너무 어렵다...
//		-> 응답 화면 만드는 부분의 표기법만 HTML로 변경하자!
//		-> 이것이 JSP 이다!(눈에 보이는 코드는 HTML, 해석은 Java)

//		Servlet -> RequestDispatcher(요청 발송자) -> forward(req,resp) ->   JSP
		
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/signUpResult.jsp");
//																JSP경로 작성 시 webapp 폴더를 기준으로 작성
